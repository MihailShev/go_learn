...
func calcWords(words []string) map[string]wordCount {
	wordCache := map[string]wordCount{}

	for _, word := range words {
		cachedWord, ok := wordCache[word]

		if ok {
			cachedWord.count++
			wordCache[word] = cachedWord
		} else {
			wordCache[word] = wordCount{word: word, count: 1}
		}
	}
...
func makeWordList(cache map[string]wordCount) []wordCount {
	wordList := make([]wordCount, 0, len(cache))
...
func printTopWords(wordList []wordCount, top int) {
	copiedWords := make([]wordCount, len(wordList))
